node {
    ...
    git credentialsId: url: 'https://github.com/mlf4aiur/stock_price.git'

    // Mark the code test 'stage'....
    stage 'Test'
    // Run the unit test
    runTests('test_stock_price.py')

    // Mark the code build 'stage'....
    stage 'Bake Docker image'
    // Run the docker build
    def v = version('stock_price.py')
    def docker_tag = "${v}-${env.BUILD_NUMBER}"
    def dockerRepoName = 'kevin/stock_price'
...
def runTests(testFile) {
    sh '''
        docker run \\
            ...
            -w /myapp \
            python:3.5.2-alpine pip install -r requirements.txt && python test_stock_price.py"
    '''
}
